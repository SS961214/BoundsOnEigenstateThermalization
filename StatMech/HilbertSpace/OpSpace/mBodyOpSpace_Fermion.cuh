...
__global__ void construct_mBodyOpSpace_kernel(mBodyOpSpace<ManyBodyFermionSpace, Scalar_>* ptr,
...
#endif
	new(ptr) mBodyOpSpace<ManyBodyFermionSpace, Scalar_>(m, sysSize, N);
	DEBUG(printf("#\tdim = %d, sysSize = %d, m = %d, N = %d\n", int(ptr->dim()), int(sysSize),
	             int(m), int(N)));
}

template<typename Scalar_>
class ObjectOnGPU< mBodyOpSpace<ManyBodyFermionSpace, Scalar_> > {
	private:
		using T   = mBodyOpSpace<ManyBodyFermionSpace, Scalar_>;
		T*  m_ptr = nullptr;
		int m_dev = -1;
