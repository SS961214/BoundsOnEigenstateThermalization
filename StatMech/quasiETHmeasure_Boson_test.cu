...
TEST_CASE("quasiETHmeasure_Boson_onGPU", "test") {
	...
	constexpr double precision = 1.0e-12;
	{
		...
		constexpr int LMin = 4;
		constexpr int LMax = 10;
		// constexpr int NMin     = 6;
		...
		for(auto L = LMin; L <= LMax; ++L)
			for(auto N = L; N <= L; ++N) {
				...
				Eigen::ArrayXXd ETHmeasure  = Eigen::ArrayXXd::Zero(dim, N + 1);
				for(auto m = 1; m <= N; ++m) {
				...
				auto const maxDiff = (ETHmeasure.col(N) - theorySum).cwiseAbs().maxCoeff();
