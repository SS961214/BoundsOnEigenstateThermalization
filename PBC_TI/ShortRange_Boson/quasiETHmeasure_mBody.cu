...
fs::path IOManager::generate_filepath(Index sysSize, Index N) const {
	return this->rootDir()
	       / ("mBodyETH/PBC_TI/ShortRange_Boson_" + std::to_string(ell) + "local_"
...
int main(int argc, char** argv) {
	...
	using StateSpace = ManyBodyBosonSpace;

	// Two-local operator without parity symmetry
	mBodyOpSpace<StateSpace, Scalar> const locOpSpace(k, ell, N);
	...
	for(sampleNo = sampleMin; sampleNo <= sampleMax; ++sampleNo) {
		...
		Eigen::MatrixX<Scalar> H = construct_globalOp(coeff, locOpSpace, globOpSpace, sector);
